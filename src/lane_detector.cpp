/*
 ...
 Description:
  This ROS 2 (Humble) node performs lane detection using OpenCV, subscribes to a camera image topic, extracts lane markings using
  HSV thresholding and Sobel edges, applies sliding-window tracking,
  fits quadratic curves to each lane line, and publishes:
   - /lanes/centerline (nav_msgs/Path)
   - /lanes/confidence (std_msgs/Float32)
   - /lanes/debug_image (sensor_msgs/Image)

 The algorithm design follows standard computer-vision practices for
 lane detection (binary masking + polynomial fit)
 ======================================================
 */
...
struct Poly2 {
  /* Poly2 struct used as paths for the robot */
  double a{0}, b{0}, c{0};
  bool valid{false};
...
static Poly2 fit_poly2(const std::vector<cv::Point>& pts) {
  ...
  cv::Mat coef;
  if (!cv::solve(A, X, coef, cv::DECOMP_SVD)) 
    return poly;
...
static void draw_poly(cv::Mat& img, const Poly2& p, const cv::Scalar& col) {
  /* Drawing the path onto the image */
...
class LaneDetectorNode : public rclcpp::Node {
  /* Lane Dector Node class for ROS to use. Subscribes to the mock camera that feeds in 
  the video of a car driving with lanes in view and publishes the centerline, confidence, and debug_image with the paths on it to wiew in RVIZ */
public:
...
private:
  void cb(const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
    ...
    // Sliding windows to collect lane pixels
    ...
    for (int x = 0; x < mid; ++x){ 
      // Count the amount of pixels for the left lane
      int v = hist.at<int>(0,x); 
      if (v > ml){ 
        ml = v; leftx = x; 
        } 
      }
      // count the amount of pixels for the right lane
    for (int x = mid; x < W; ++x){ 
      int v = hist.at<int>(0,x); 
      if (v > mr){ 
        mr = v; 
    ...
    const int nwindows = 9; // number of windows (vertical stacked on one another like boxes)
    const int winH = H / nwindows; // height of the sliding windows
    int margin = 60, minpix = 50; // minimum pixels and the margin

    std::vector<cv::Point> L, R; // vector of points for left and right lanes
    for (int w = 0; w < nwindows; ++w) {
      ...
      // all non zero points are white points (0 = black colorwise)
      cv::findNonZero(mask(cv::Rect(lxlo, ylo, lxhi - lxlo + 1, yhi - ylo)), nl); 
      cv::findNonZero(mask(cv::Rect(rxlo, ylo, rxhi - rxlo + 1, yhi - ylo)), nr);

      // Counting up all the not black pixels for left lane
      for (auto &p : nl) 
        L.emplace_back(p.x + lxlo, p.y + ylo);

      // Counting up all the not black pixels for the right lane
      for (auto &p : nr) 
        R.emplace_back(p.x + rxlo, p.y + ylo);

      // Gives where we are placing the next sliding box based on the average amount of white pixels (so it makes the center of the box follow the path of white pixels)
      // leftx is like the coordinate point of where to put the next box
      ...
      // Gives where we are placing the next sliding box based on the average amount of white pixels (so it makes the center of the box follow the path of white pixels)
      // rightx is like the coordinate point of where to put the next box
    ...
    // Fit quadratics x(y) for left/right
    ...
    if (pl.valid) 
      draw_poly(dbg, pl, cv::Scalar(0,255,0));   // green
    if (pr.valid) 
...
int main(int argc, char** argv) {
  ...
  rclcpp::spin(std::make_shared<LaneDetectorNode>()); // spin up the lane detector node
  rclcpp::shutdown();
  return 0;
}
