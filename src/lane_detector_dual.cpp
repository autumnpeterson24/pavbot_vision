...
//     // Blob Stuff
//     declare_parameter<int>("min_blob_area", 0);
//     declare_parameter<float>("min_blob_aspect_ratio", 2.0f);
...
//     // Blob Stuff
//     get_parameter("min_blob_area", min_blob_area_);
//     get_parameter("min_blob_aspect_ratio", min_blob_aspect_ratio_);
...
// private:
//   // Connected Components Blob Discarding
//   int min_blob_area_{500};
//   float min_blob_aspect_ratio_{2.0f};
...
//     // NEW MICHAEL STUFF (from test_coonectedcomponents.cpp in opencv's git + random interwebs)
//     cv::Mat labels, stats, centroids;
//     int num_labels = cv::connectedComponentsWithStats(mask, labels, stats, centroids, 8, CV_32S);
    
//     // The ol' discard (ADJUST BASED ON LOWER STUFF TOO!!! BE CONSISTENT)
//     const int H = mask.rows;
//     const int W = mask.cols;
//     const int x_lo = is_left_camera ? 0   : W/2;
//     const int x_hi = is_left_camera ? W/2 : W;
    
//     for (int i = 0; i < num_labels; ++i) {
//         const int area = stats.at<int>(i, cv::CC_STAT_AREA);         // Area of blob
//         const int bx = stats.at<int>(i, cv::CC_STAT_LEFT);           // Corner (?)
//         const int bw = stats.at<int>(i, cv::CC_STAT_WIDTH);          // Width
//         const int bh = stats.at<int>(i, cv::CC_STAT_HEIGHT);         // Height
//         const float ar = (bw > 0) ? (float)bh / (float)bw : 0.0f;    // Aspect ratio
//         const int cx = (int)centroids.at<double>(i, 0);              // Centroids
    
//         const bool area_ok = (area >= min_blob_area_);
//         const bool ar_ok   = (ar   >= min_blob_aspect_ratio_);
//         //const bool side_ok = (cx >= x_lo && cx < x_hi);
//         bool side_ok = true;

//         if (!area_ok || !ar_ok || !side_ok) {
//             // Zero out this blob in the mask (banished)
//             mask.setTo(0, labels == i);
//         }
//     }
    
//     // END NEW MICHAEL STUFF

//       cv::morphologyEx(mask, mask, cv::MORPH_CLOSE,
//                      cv::getStructuringElement(cv::MORPH_RECT, {4,5}));

//     cv::morphologyEx(mask, mask, cv::MORPH_OPEN,
//                      cv::getStructuringElement(cv::MORPH_RECT, {3,4}));
